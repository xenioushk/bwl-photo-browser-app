...
const Photos = ({ albumId, albumTitle }) => {
  ...
  const { data, isLoading, isError, error } = usePhotos(page, limit, albumId)
  ...
  return (
    <div className="container px-4 mx-auto items-center md:px-0 mt-5">
      ...
      {!isLoading && allPhotos.length === 0 && (
        <div className="grid grid-cols-1 text-center mt-6">No photos available!</div>
      )}

      {!hasMorePhotos && allPhotos.length > 0 && (
        <div className="grid grid-cols-1 text-center mt-6">No more photos available!</div>
      )}
    </div>
  )
}
